...
jQuery(document).ready(function() {
    ...
    */
    ...
    var getAllCheckBoxesMappedForDeleteBtn = function(deleteBtn) {
        var checkBoxes = deleteBtn.parent().next().find("input[type=checkbox]");
        if(checkBoxes.length==0 && deleteBtn.hasClass('btn')) {
            var boxParent = deleteBtn.parents(".box")[0];
            checkBoxes = jQuery(boxParent).find("input[type=checkbox]");
            
        }
        return checkBoxes;
    };
    var getCheckedCheckBoxesMappedForDeleteBtn = function(deleteBtn) {
        var checkBoxes = deleteBtn.parent().next().find("input[type=checkbox]:checked");
        if(checkBoxes.length==0 && deleteBtn.hasClass('btn')) {
            var boxParent = deleteBtn.parents(".box")[0];
            checkBoxes = jQuery(boxParent).find("input[type=checkbox]:checked");
        }
        return checkBoxes;
    };    
    var setUpCommonUserInterfaceEnhancements = function() {
        ...
        */
        ...
        jQuery('button.delete,button[data-warning]').bind('click keypress', function() {
            ...
            var checkBoxes = getAllCheckBoxesMappedForDeleteBtn(clickedButton);
            
                if (checkBoxes.length > 0) {
                // check is there selected checkboxes if not - alert
                var checkedCheckBoxes = getCheckedCheckBoxesMappedForDeleteBtn(clickedButton);
    ...
    var setUpListFilters = function() {
        ...
        findControlBar = function(table) {
            //new styles
            if(jQuery(table).hasClass('filtered')) {
                ...
                //old styles - remove only when we've switched to new bootstrap style
